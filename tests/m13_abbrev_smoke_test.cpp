...
static std::string runSequence(farolkey::core::Engine& eng, const std::string& seq) {
    std::string commits;
    for (char c : seq) {
...
static std::string tmpDictPath() {
    return (fs::temp_directory_path() / "farolkey_m13_test_dict.json").string();
}
...
TEST(M13Schema, LoadAbbrevMode) {
    ...
    const auto d = farolkey::core::UserDict::loadFromFile(p);
    ASSERT_EQ(d.size(), 4u);

    EXPECT_EQ(d.lookup("ko")->abbrev_mode,   farolkey::core::AbbrevMode::Vi);
    EXPECT_EQ(d.lookup("doex")->abbrev_mode, farolkey::core::AbbrevMode::En);
    EXPECT_EQ(d.lookup("btv")->abbrev_mode,  farolkey::core::AbbrevMode::Both);
    EXPECT_EQ(d.lookup("old")->abbrev_mode,  farolkey::core::AbbrevMode::Vi);  // backward compat
    fs::remove(p);
}

TEST(M13Schema, UpsertAndRemove) {
    farolkey::core::UserDict d;
    d.upsert({"a", "alpha", farolkey::core::AbbrevMode::Vi});
    d.upsert({"b", "beta",  farolkey::core::AbbrevMode::En});
    EXPECT_EQ(d.size(), 2u);

    const bool replaced = d.upsert({"a", "ALPHA", farolkey::core::AbbrevMode::Both});
    EXPECT_TRUE(replaced);
    EXPECT_EQ(d.lookup("a")->expansion, "ALPHA");
    EXPECT_EQ(d.lookup("a")->abbrev_mode, farolkey::core::AbbrevMode::Both);
...
TEST(M13Schema, SaveAndReload) {
    ...
    farolkey::core::UserDict d;
    d.upsert({"ko",   "không",         farolkey::core::AbbrevMode::Vi});
    d.upsert({"doex", "docker exec",   farolkey::core::AbbrevMode::En});
    d.upsert({"btv",  "Ban Tổ chức",   farolkey::core::AbbrevMode::Both});
    ASSERT_TRUE(d.saveToFile(p));

    const auto d2 = farolkey::core::UserDict::loadFromFile(p);
    ASSERT_EQ(d2.size(), 3u);
    EXPECT_EQ(d2.lookup("ko")->expansion,   "không");
    EXPECT_EQ(d2.lookup("doex")->abbrev_mode, farolkey::core::AbbrevMode::En);
    EXPECT_EQ(d2.lookup("btv")->abbrev_mode,  farolkey::core::AbbrevMode::Both);
    fs::remove(p);
    fs::remove(p + ".bak");
}
...
TEST(M13Engine, ViModeEntryExpandsInVi) {
    ...
    auto cfg = farolkey::config::defaultConfig();
    cfg.userDictPath = p;
    cfg.enableUserDictionary = true;
    farolkey::core::Engine eng(cfg);
