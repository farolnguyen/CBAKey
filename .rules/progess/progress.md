...
### Milestone M22 — Output Charset (TCVN3 / CP1258 / VISCII) ✅ DONE (M22.1–M22.5)
...
| ID | Hạng mục | Deliverable | Phụ thuộc | Exit criteria |
|----|----------|-------------|-----------|---------------|
| M22.1 ✅ | Output charset infrastructure | Enum `OutputCharset`; config `outputCharset`; hook trước `commitString`; no-op khi Unicode | — | No-op verified; 0 regression | **DONE 2026-06-08** |
| M22.2 ✅ | TCVN3 (ABC) table | `src/core/charset/tcvn3_table.cpp` — 122 entries NFC→byte; `tests/charset_smoke_test.cpp` 10/10 pass | M22.1 | Round-trip ≥100 ký tự | **DONE 2026-06-08** |
| M22.3 ✅ | CP1258 (Windows-1258) table | `src/core/charset/cp1258_table.cpp` — 30 direct + 104 decompose entries; `tests/charset_smoke_test.cpp` +7 tests, 17/17 pass | M22.1 | Round-trip ≥100 ký tự | **DONE 2026-06-10** |
| M22.4 ✅ | VISCII table | `src/core/charset/viscii_table.cpp` — 134 entries NFC→byte, full alphabet (no decompose); `tests/charset_smoke_test.cpp` +9 tests, 26/26 pass | M22.1 | Round-trip ≥100 ký tự | **DONE 2026-06-10** |
| M22.5 ✅ | Settings UI | Dropdown "Đầu ra → Bảng mã đầu ra" trong `farolkey-settings`; warning label non-Unicode; save/reload config key `OutputCharset` | M22.2, M22.3, M22.4 | Save/reload đúng; warning hiển thị khi chọn non-Unicode; không conflict với input method dropdown | **DONE 2026-06-10** |
...
- 2026-06-08: **M22.1 ✅ — Output Charset Infrastructure:** `include/farolkey/core/charset/output_charset.h` (`OutputCharset` enum + `charsetConvert` API); `src/core/charset/output_charset.cpp` (UTF-8 decoder `nextCodepoint`, `convertWithTable`, dispatch switch); stubs `tcvn3_table.cpp`/`cp1258_table.cpp`/`viscii_table.cpp`; `FCITX_CONFIG_ENUM(FarolKeyOutputCharset)` + `Option<FarolKeyOutputCharset> outputCharset` trong config; `toOutputCharset()` bridge + `commitConverted()` helper trong engine; thay thế 4 `ic->commitString()` call → `commitConverted(ic, x)`; cập nhật `CMakeLists.txt`. Unicode no-op verified; 0 regression.
- 2026-06-08: **M22.2 ✅ — TCVN3 (ABC) Table:** Thay thế stub `tcvn3_table.cpp` bằng bảng 122 entry NFC→byte sinh từ `iconv TCVN5712-1:1993` (bỏ combining marks U+0300/0301/0303/0309/0323 và NBSP vì engine output NFC precomposed); nhóm: base chars, A+tone, Â-family, Ă-family, Ê-family, Ô-family, Ơ-family, Ư-family, Y+tone; ký tự không có trong chuẩn (Ứ U+1EE8, Ừ, Ủ uppercase horn vowels) fallback UTF-8. `tests/charset_smoke_test.cpp`: 10 test, 10/10 pass ("tiếng"→TCVN3, "việt", "được", "hoặc"; unmappable passthrough; mixed ASCII).
- 2026-06-10: **M22.3 ✅ — CP1258 (Windows-1258) Table:** CP1258 không có byte đơn cho phần lớn nguyên âm có dấu (â/ê/ô/ơ/ư + thanh điệu, ã/ì/ò/õ/ý…) — encode bằng base-letter byte + 1 trong 5 byte combining-diacritic (0xCC grave/0xEC acute/0xD2 hook/0xDE tilde/0xF2 dot-below). Thêm `cp1258DecomposeTable()` (`unordered_map<char32_t, pair<uint8_t,uint8_t>>`, 104 entries) bên cạnh `cp1258Table()` (30 direct single-byte entries); cả hai sinh từ `iconv WINDOWS-1258` byte-scan + NFD recombination 2 chiều. `output_charset.cpp`: thêm `convertCp1258()` — thử direct table trước, sau đó decompose table (2 byte), cuối cùng fallback UTF-8. `tests/charset_smoke_test.cpp`: +7 test (`charset_cp1258.*`), 17/17 pass. Full corpus + suite: 18/19 (1 fail `engine_smoke_test` pre-existing, không liên quan).
- 2026-06-10: **M22.5 ✅ — Settings UI dropdown "Output encoding":** `src/gui/farolkey-settings`: thêm `_GENERAL_DEFAULTS['OutputCharset'] = 'Unicode'`; section mới "Output"/"Đầu ra" trong tab General với `Gtk.ComboBoxText` 4 option (Unicode (UTF-8) / TCVN3 (ABC) / CP1258 (Windows-1258) / VISCII) + warning hint "⚠ TCVN3/CP1258/VISCII gửi byte mã cũ — chỉ ứng dụng XIM cũ đọc được..." chỉ hiện khi chọn non-Unicode (`set_no_show_all(True)` + `set_visible()` trên `changed` signal — lưu ý `win.show_all()` ở entry point sẽ ghi đè `set_visible(False)` nếu không set `no_show_all`). Cập nhật `_on_general_save`/`_on_general_reset`/reset-confirm để đọc/ghi key `OutputCharset`. Thêm 3 msgid mới vào `locales/{en,vi}/LC_MESSAGES/farolkey.po` (Output, Output encoding, warning text), recompile `.mo`. Verified bằng GUI screenshot: combo + warning hiện/ẩn đúng khi đổi giá trị và khi load `OutputCharset=TCVN3` từ config có sẵn. Deploy: `cp src/gui/farolkey-settings ~/.local/bin/` + `.mo` → `~/.local/share/farolkey/locales/`. Full suite: 18/19 (1 fail `engine_smoke_test` pre-existing, không liên quan). **→ M22 hoàn thành toàn bộ (M22.1–M22.5).**
- 2026-06-10: **M22.4 ✅ — VISCII Table:** VISCII (RFC 1456) gán toàn bộ 134 chữ cái tiếng Việt vào single byte — phủ kín cả Ứ/Ừ/Ử/Ữ/Ự uppercase mà TCVN3 thiếu — bằng cách tái sử dụng 6 vị trí byte control-char (0x02→Ẳ, 0x05→Ẵ, 0x06→Ẫ, 0x14→Ỷ, 0x19→Ỹ, 0x1E→Ỵ) cộng toàn bộ dải 0x80–0xFF. Sinh bảng `viscii_table.cpp` (134 entries NFC→byte) từ `iconv VISCII` byte-scan 0x00–0xFF; không cần decompose, dùng lại `convertWithTable()` có sẵn (dispatch VISCII trong `output_charset.cpp` không đổi). `tests/charset_smoke_test.cpp`: +9 test (`charset_viscii.*`) gồm base chars, tones, circumflex/horn family, words, control-char reassignment, full-alphabet coverage (Ứ), unmappable passthrough — 26/26 pass. Full suite: 18/19 (1 fail `engine_smoke_test` pre-existing, không liên quan).
- 2026-06-10: **M22.5 fix — Sắp xếp lại section "Output" trong Settings UI:** Theo yêu cầu user, di chuyển section "Output"/"Đầu ra" (combo Output encoding + warning) lên ngay sau section "Input", trước "Display" (trước đó nằm dưới "Display"). Chỉ đổi thứ tự khối code trong `_build_general_tab()`, không đổi logic. Verified bằng GUI screenshot. Deploy: `cp src/gui/farolkey-settings ~/.local/bin/`.
- 2026-06-10: **Test E2E M22 — phát hiện + fix gap deploy:** `libfarolkey.so` đang chạy (mtime 06-08) chưa hề chứa code M22.1–M22.4 (`nm -D` thiếu `charsetConvert`/`visciiTable`) — toàn bộ Output Charset chưa từng hoạt động trên máy thật. Rebuild `cmake --build build` + deploy `libfarolkey.so` vào `~/.local/lib/{fcitx5,x86_64-linux-gnu/fcitx5}/` + `fcitx5 -r -d`. Verify bằng debug log tạm trong `commitConverted()`: `OutputCharset=TCVN3` load đúng từ `farolkey.conf`, `charsetConvert("ă")` → đúng 1 byte `0xA8`. Tuy nhiên byte non-UTF-8 bị fcitx5-gtk/DBus drop hoàn toàn (không hiển thị gì) khi commit tới gnome-terminal — đúng như thiết kế (chỉ dành cho XIM raw cũ, không qua DBus), nhưng "mất ký tự" chứ không chỉ "garbled" như text cảnh báo M22.5 mô tả. Đã xoá debug code, restore `OutputCharset=Unicode`, verify lại Unicode commit OK ("ă" → `c4 83`).
- 2026-06-10: **M22.5 fix — Sửa text cảnh báo Output charset + thêm tiếng Anh cho "? Hướng dẫn":** (1) Cập nhật msgid/msgstr cảnh báo trong `farolkey-settings` + `locales/{en,vi}/LC_MESSAGES/farolkey.po`: từ "will show garbled text" → "will silently disappear instead of being typed" (đúng với phát hiện E2E ở trên — DBus drop byte non-UTF-8 chứ không hiển thị sai); recompile `.mo`. (2) `MethodReferenceDialog` ("? Hướng dẫn"/"? Guide"): tách `_METHOD_REFS` → `_METHOD_REFS_VI` (giữ nguyên nội dung) + `_METHOD_REFS_EN` mới (dịch đầy đủ desc/section/table/notes cho cả 9 phương thức gõ); `_METHODS_VI`/`_METHODS_EN` + `_REFS` chọn theo `_STARTUP_LANGUAGE`. Đồng thời sửa key "Free Layout (Kiểu gõ Tuỳ biến)" → "Tuỳ biến"/"Free Layout" để khớp `_FL_DISPLAY_NAME` của combo (trước đó lookup luôn miss → mặc định về Telex). Verified bằng script kiểm tra cấu trúc dữ liệu (9/9 method khớp `_METHODS`/`_REFS` cho cả VI/EN, gettext trả đúng cảnh báo cho cả 2 ngôn ngữ). Deploy: `cp src/gui/farolkey-settings ~/.local/bin/` + `.mo` → `~/.local/share/farolkey/locales/{en,vi}/LC_MESSAGES/`.
